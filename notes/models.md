...
Proper paragraph explanation + one simple example + easy memory trick.
...
Suppose we want to predict whether a customer will leave a company. Instead of using one Decision Tree, Random Forest creates many trees:
...
Suppose we want to classify a fruit as an apple or orange based on its weight and size. For a new fruit, KNN finds the closest fruits in the training data.

If $K = 5$, nearest neighbors:
...
Suppose an e-commerce company has customer data based on spending and number of purchases. We don’t know the customer categories beforehand, so we set:
...
| Goal       | Predicts a new data point  | Creates groups/clusters     |
...
Suppose we want to determine whether an email is Spam or Not Spam. The email contains words such as:
...
$$
P(\text{Spam} \mid \text{words}) = \text{high}, \quad P(\text{Not Spam} \mid \text{words}) = \text{low}
$$
